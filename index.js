...
async function run() {
  try {
    ...
    app.post('/foodsDatabase',async(req,res)=>{
      const foodData=req.body;
      const result=await foodsDatabaseCollection.insertOne(foodData)
      res.send(result)
    })
   
    app.get('/foodsDatabase',async(req,res)=>{
      const result=await foodsDatabaseCollection.find().toArray()
      res.send(result)
    })
    app.get('/foodsDatabase/:id',async(req,res)=>{
      const id=req.params.id;
      const result=await foodsDatabaseCollection.findOne({_id:new ObjectId(id)})
      res.send(result)
    })
    app.patch('/foodsDatabase/:id',async(req,res)=>{
      const id=req.params.id;
      const updatedData=req.body;
      const result=await foodsDatabaseCollection.updateOne(
        {_id:new ObjectId(id)},
        {$set:updatedData}
      )
      res.send(result)
    })
    app.delete('/foodsDatabase/:id',async(req,res)=>{
      const id=req.params.id;
      const result=await foodsDatabaseCollection.deleteOne({_id:new ObjectId(id)})
      res.send(result)
    })
